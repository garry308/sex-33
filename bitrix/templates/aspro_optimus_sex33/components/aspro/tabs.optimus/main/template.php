		...
		<div class="tab_slider_wrapp specials <?=$class_block;?> best_block clearfix ajax_load">
			...
			<ul class="tabs_content" data-url="<?=$url?>">
				<?foreach($arResult["TABS"] as $code=>$title){?>
					...
					$signedFilter = $signer->signParameters($this->__component->getName(), $GLOBALS[$arParams["FILTER_NAME"]]);
					?>
					<li class="tab <?=$code?>_wrapp <?=($j++ ==1 ? "cur" : "");?>" data-code="<?=$code?>" data-col="<?=$col;?>"  data-filter="<?=$signedFilter;?>">
